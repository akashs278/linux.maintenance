...
update_pacman() {
    if command -v pacman &> /dev/null; then
        print_header "Synchronizing & Upgrading Packages"
        run_command "Pacman Update" $SUDO pacman -Syu --noconfirm
        
        print_header "Removing Orphaned Packages"
        if pacman -Qdtq &> /dev/null; then
            # We disable set -e temporarily in run_command invocation because pacman -Qdtq could return nothing
            # but we already checked it above, so it's fine.
            run_command "Pacman Remove Orphans" $SUDO pacman -Rns $(pacman -Qdtq) --noconfirm
        else
            print_info "No orphaned packages to remove."
        fi
        
        print_header "Cleaning Package Cache"
        run_command "Pacman Clean Cache" $SUDO pacman -Sc --noconfirm
    fi
}
